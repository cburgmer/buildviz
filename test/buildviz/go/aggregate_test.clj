...
(deftest test-aggregate-jobs-for-stage
  (testing "outcome"
    (testing "should set outcome to pass for all jobs passing"
      (is (= "pass"
             (:outcome (aggregate/aggregate-jobs-for-stage (a-stage-instance [(a-job-instance "pass" "1")
    ...
    (testing "should set outcome to pass after re-running a a now passing job"
      (is (= "pass"
             (:outcome (aggregate/aggregate-jobs-for-stage (a-stage-instance [(a-job-instance "pass" "2")
  ...
  (testing "build time"
    (testing "should take lowest start time"
      (is (= 100
             (:start (aggregate/aggregate-jobs-for-stage (a-stage-instance [(a-job-instance 200 400 "1")
                                                                             (a-job-instance 100 300 "1")] "1"))))))

    (testing "should take highest end time"
      (is (= 400
             (:end (aggregate/aggregate-jobs-for-stage (a-stage-instance [(a-job-instance 200 400 "1")
                                                                           (a-job-instance 100 300 "1")] "1"))))))

    (testing "should only consider times of re-run jobs"
      (let [aggregated-job (aggregate/aggregate-jobs-for-stage (a-stage-instance [(a-job-instance 200 300 "2")
  ...
  (testing "xml"
    (testing "should merge JUnit XML lists"
      (is (= ["testsuite 1" "second suite" "another testsuite"]
             (:junit-xml (aggregate/aggregate-jobs-for-stage (a-stage-instance [(a-job-instance-with-xml ["testsuite 1" "second suite"] "1")
