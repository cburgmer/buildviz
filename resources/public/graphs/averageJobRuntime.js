(function(
            ...
            .map(function(job) {
                const averageRuntime = job.averageRuntime,
                    runtime = averageRuntime
                        ? " (" + utils.formatTimeInMs(averageRuntime) + ")"
                        : "";
    ...
    const renderGraph = function(jobEntries, svg) {
        ...
        const selection = svg
            .selectAll("g")
            .data(
                skipParentNodes(treemap.nodes({ children: builds })),
                function(d) {
                    return d.name;
                }
            );
        ...
        node.append("rect").style("fill", function(d) {
            return color(d.name);
        });
    ...
    const timespanSelector = timespanSelection.create(
        ...
        description = graphDescription.create({
            description:
                "Average runtime by job for the selected interval. The job's runtime is calculated as time between start and end of its builds.",
            answer: ["Where is most of the time spent?"],
            legend:
                "Size: average runtime, color: job (similar colors for job group)",
            csvSource: "jobs.csv"
        }),
