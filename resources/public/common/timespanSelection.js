const timespanSelection = (function (d3) {
    ...
    module.create = function (selectedSpan) {
        ...
        const timespanList = container
            .append("div")
            .attr("class", "timespanSelection")
            .append("ol")
            .attr("class", "timespanList");

        const timespans = Object.keys(module.timespans).map(function (
            spanName
        ) {
            return module.timespans[spanName];
        });

        const selection = timespanList
            .selectAll(".item")
            .data(timespans, function (d) {
                return d.label;
            });

        const updateSelection = function () {
            currentlySelected.text(selectedSpan.shortLabel);
            selection.classed("selected", function (span) {
